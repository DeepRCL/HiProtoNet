...
from src.models.Video_XProtoNet import Video_XProtoNet
from src.utils import lorentz as L
from src.utils.model_utils import get_prototype_class_identity

class Hyper_Video_XProtoNet(Video_XProtoNet):
    def __init__(
        self,
        ...
        init_weights=True,
        cls_method: str = "broad",  # broad, local, or both
        ...
        **kwargs
        ...
        super(Hyper_Video_XProtoNet, self).__init__(init_weights=False, **kwargs)
        ...
        self.local_prototype_shape = (self.num_local_prototypes, self.prototype_shape[1], 1, 1, 1)
        # shape  (PL=num_classes*L, D, 1, 1)
        self.local_prototype_vectors = nn.Parameter(torch.__dict__[self.prot_init_method](self.local_prototype_shape)*local_prot_donut,
                                                     requires_grad=True)
        ...
        # Need global pooling to get it to 1x1x1
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1, 1))  # Reduces (T, 7, 7) to (1, 1, 1)
    ...
    def forward_detailed(self, x):
        ...
        if self.local_prototype_method == "attn":
            local_attn_map = self.get_local_attn_map(x)  # shape (N, L, 1, T, H, W)
            local_feats_extracted = (local_attn_map * feature_map).sum(dim=3).sum(dim=3).sum(dim=3)  # shape (N, L, D)
            # TODO need to do average! not sum! eh?
            # local_feats_extracted = (local_attn_map * feature_map).mean(dim=(3, 4, 5))  # shape (N, L, D)
    ...
    def get_local_attn_map(self, x):
        ...
        if self.local_prototype_method == "last_layer":
            ...
            local_attn_map = self.local_attn_module(x)  # shape (N, L, T, H, W)

            # # Option 1: Absolute value, TODO double check again
            # local_attn_map = torch.abs(local_attn_map).unsqueeze(2)  # shape (N, L, 1, T, H, W)

            # Option 2: Softmax!
            # n, L, t, h, w = local_attn_map.shape
            # local_attn_map = local_attn_map.reshape((n, L, -1))
            # temperature = 4.0
            # local_attn_map = self.om_softmax(local_attn_map/temperature).reshape((n, L, t, h, w)).unsqueeze(2)  # shape (N, L, 1, T, H, W)

            # Option 3: Layernorm + sigmoid!  BAD
            # N, L, T, H, W = local_attn_map.shape
            # local_attn_map = local_attn_map.view(N * L, T, H, W)  # Flatten N and P into a single dimension
            # local_attn_map = F.layer_norm(local_attn_map, (T, H, W))  # Normalize over H and W
            # local_attn_map = local_attn_map.view(N, L, T, H, W)  # Reshape back to original dimensions
            # local_attn_map = F.sigmoid(local_attn_map)
            ...
            # localattn_map = F.sigmoid(local_attn_map).unsqueeze(2)  # shape (N, L, 1, T, H, W)

            # Option 6: min-max norm! subtract min, divide by max! to be in range [0-1]
            local_attn_map = local_attn_map - local_attn_map.min(dim=2, keepdim=True)[0].min(dim=3, keepdim=True)[0].min(dim=4, keepdim=True)[0]
            epsilon = 1e-8
            local_attn_map = local_attn_map / (epsilon + local_attn_map.max(dim=2, keepdim=True)[0].max(dim=3, keepdim=True)[0].max(dim=4, keepdim=True)[0])
            local_attn_map = local_attn_map / (epsilon + local_attn_map.sum(dim=(2,3,4), keepdim=True))
            local_attn_map = local_attn_map.unsqueeze(2)  # shape (N, L, 1, T, H, W)
...
def construct_Hyper_XProtoNet_Video(
    base_architecture,
    pretrained=True,
    img_size=224,
    ...
    ** kwargs
    ...
    cnn_backbone = base_architecture_to_features[base_architecture](pretrained=pretrained, last_layer_num=backbone_last_layer_num)
