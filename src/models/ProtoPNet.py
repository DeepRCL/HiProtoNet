...
from src.models.resnet_features import (
    resnet18_features,
    resnet34_features,
    resnet50_features,
    resnet101_features,
...
class PPNet(nn.Module):
    def __init__(
        ...
        first_add_on_layer_in_channels = self.get_cnn_backbone_out_channels(self.cnn_backbone)
        ...
        if add_on_layers_type == "bottleneck":
            ...
            self.add_on_layers = nn.Sequential(
                ...
                nn.BatchNorm2d(self.prototype_shape[1]),  # TODO Explore if needed
                nn.ReLU(),
                nn.Conv2d(
        ...
        if init_weights:
            ...
            self.set_last_layer_incorrect_connection(layer_name="last_layer", incorrect_strength=-0.5)

    def conv_features(self, x):
        """
        the feature input to prototype layer
        """
        x = self.cnn_backbone(x)  # [N, 3, 224, 224] --> [N, 1024, 7, 7]
        x = self.add_on_layers(x)  # shape (N, 512, 7, 7)
        return x

    @staticmethod
    def get_cnn_backbone_out_channels(cnn_backbone):
        features_name = str(cnn_backbone).upper()
        if "RESNET2P1D" in features_name:
            return [i for i in cnn_backbone.modules() if isinstance(i, nn.Conv3d)][-1].out_channels
        elif features_name.startswith("VGG") or features_name.startswith("RES") or features_name.startswith("WIDE_RES"):
            return [i for i in cnn_backbone.modules() if isinstance(i, nn.Conv2d)][-1].out_channels
        elif features_name.startswith("DENSE"):
            return [i for i in cnn_backbone.modules() if isinstance(i, nn.BatchNorm2d)][-1].num_features
        else:
    ...
    def prototype_distances(self, x):
        ...
        x is the raw input
        """
        conv_features = self.conv_features(x)  # shape (N, 512, 7, 7)
        distances = self._l2_convolution(conv_features)  # shape (N, P, 7, 7)
        return distances
    ...
    def forward(self, x):
        ...
        min_distances = min_distances.view(-1, self.num_prototypes)  # shape (N, P)
        prototype_activations = self.distance_2_similarity(min_distances)  # shape (N, P)
    ...
    def push_prototypes(self, prototype_update):
    ...
    def __repr__(self):
        # PPNet(self, cnn_backbone, img_size, prototype_shape,
    ...
    def set_last_layer_incorrect_connection(self, layer_name="last_layer", incorrect_strength=-0.5):
        """
        the incorrect strength will be actual strength if -0.5 then input -0.5
        """
        prototype_class_identity = self.get_last_layer_prot_class_identity(layer_name=layer_name)
        ...
        if layer_name == "last_layer_local":
            self.last_layer_local.weight.data.copy_(
                correct_class_connection * positive_one_weights_locations
                + incorrect_class_connection * negative_one_weights_locations
            )
    ...
    def _initialize_weights(self, layer):
        for m in layer.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Conv3d):
                ...
                elif self.feat_range_type == "CLIP-style":
                    # CLIP-style initialization: Truncated Normal with std=0.02
                    nn.init.trunc_normal_(m.weight, mean=0.0, std=0.02)  # Truncated Normal
                else:
                    raise(ValueError(f"feat_range_type {self.feat_range_type} is not supported"))

                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)  # Bias to 0
