...
class wide_resnet50_2(nn.Module):
    def __init__(self, pretrained=True, last_layer_num=-2, **kwargs):
        """
        :param pretrained: to load the pretrained weights or not
        :param last_layer_num: for input shape (3,112,112)
            if -2 => output shape [-1, 2048, 4, 4].
        """
        super().__init__()
        model = torch.hub.load('pytorch/vision:v0.10.0', 'wide_resnet50_2', pretrained=True)

        # keep the layers up to layer4 by removing the average pool and the final fc layer
        self.backbone = nn.Sequential(*(list(model.children())[:last_layer_num]))
