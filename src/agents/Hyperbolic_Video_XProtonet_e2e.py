"""
Video-based Hyperbolic XProtoNet agent
end-to-end training procedure, inherits the image-based Hyperbolic XProtoNet-e2e agent.
"""
import os
import numpy as np
import pandas as pd
...
from src.agents.Hyperbolic_XProtoNet_e2e import Hyperbolic_XProtoNet_e2e
...
class Hyperbolic_Video_XProtoNet_e2e(Hyperbolic_XProtoNet_e2e):
    ...
    def run_epoch(self, epoch, optimizer=None, mode="train"):
        ...
        # Diversity Metric        # TODO CHECK IF IT WORKS
            ...
            for i in iterator:
                ...
                if self.model.cls_method == "both":
                    ...
                    if self.config["abstain_class"]:
                        ...
                        y_pred_prob_broad = logit_broad[:, : self.model.num_classes - 1].softmax(dim=1).cpu()
                        y_pred_prob_local = logit_local[:, : self.model.num_classes - 1].softmax(dim=1).cpu()
                    else:
                ...
                sparsity_batch = getattr(self, f"{mode}_sparsity_80")(part_min_lorentz_distances).item()  # TODO FIX , has to be simscores! not distances

                # Determine the top 5 most similar prototypes to data
                # sort similarities in descending order
                # TODO FIX , has to be simscores! not distances
                sorted_similarities, sorted_indices = torch.sort(part_min_lorentz_distances[:, :num_class_prototypes].detach().cpu(),
                                                                 descending=True)
                ...
                if self.config["abstain_class"]:
                    ...
                    sorted_similarities, sorted_indices = torch.sort(
                        part_min_lorentz_distances[:, num_class_prototypes:].detach().cpu(), descending=True
                    )
                    # Add the type 5 most similar prototypes to the count array
                    np.add.at(count_array[num_class_prototypes:], sorted_indices[:, :2], 1)

                # TODO FIX , has to be simscores! not distances
                simscore_cumsum += part_min_lorentz_distances.sum(dim=0).detach().cpu()
