...
def load_data_and_model_products(model, dataloader, mode, data_config, abstain_class, root_dir_for_saving, log=print, local_prots=False):
    """
    ...
    if os.path.exists(data_dict_path) and os.path.exists(model_products_path):
        ...
        filenames = []
        if local_prots:
            fc_layer_weights = model.last_layer_local.weight.detach().cpu().numpy()
        else:
            fc_layer_weights = model.last_layer.weight.detach().cpu().numpy()

        print(fc_layer_weights.shape)
        ...
        for i in iterator:
            ...
            with (torch.no_grad()):
                ...
                if len(returned) == 4:  # for euclidean models
                    ...
                    if local_prots:  # L is 1 when  local_feat_method='1_per_input' and is PL when 1_per_class
                    ...
                    else: # shape (N, PL, D)   # shape (N, PL)  # shape (N, PL, 1, H, W)
...
def get_heatmap(rescaled_occurrence_maps):
    ...
    prots_heatmaps = prots_heatmaps[..., ::-1]  # [RGB]
