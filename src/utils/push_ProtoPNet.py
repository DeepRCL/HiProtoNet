...
def push_prototypes(
    ...
    assert(push_local_prots==False)  # For now, only use xprotonet version of pushing for local pushing
    # if push_local_prots:
    ...
    if push_local_prots:
        ...
        n_prototypes = model.num_local_prototypes
    ...
    for push_iter in iterator:
        ...
        update_prototypes_on_batch(
            ...
            push_local_prots=push_local_prots,
        )
    ...
    if replace_prototypes:
        ...
        model.push_prototypes(prototype_update)
    end = time.time()
...
def update_prototypes_on_batch(
    ...
    with torch.no_grad():
        ...
        returned = model.push_forward(search_batch)
        if len(returned) == 2:
            protoL_input_torch, proto_dist_torch = returned
        elif len(returned) == 7:
            (protoL_input_torch, _, proto_dist_torch,
            local_features_to_track, local_feat_prot_lorentz_distance, local_attn_map,
            logits) = returned
            if push_local_prots:  # L is 1 when  local_feat_method='1_per_input' and is PL when 1_per_class
                raise("shouldn't call push_prototypes for local prototypes of Protopnet. Use the one for XProtoNet")
                protoL_input_torch = local_features_to_track   # shape (N, PL, D)
                proto_dist_torch = local_feat_prot_lorentz_distance  # shape (N, PL)
                occurrence_map_torch = local_attn_map   # shape (N, PL, 1, H, W)
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            proto_act_img_j = model.distance_2_similarity(torch.from_numpy(proto_dist_img_j), max_dist).numpy()
...
def save_global_bests(
    ...
    if dir_for_saving_prototypes is not None:
        # ############################################
        # # to reduce the visualization time of prototypes, we visualize only some of them
        # P = len(prototypes_to_plot)
        # keep_idx = 2
        # if num_classes > 2*keep_idx:
        #     selected_classes = list(range(keep_idx)) + random.sample(range(keep_idx, num_classes), keep_idx)
        # else:
        #     selected_classes = range(num_classes)
        # # select indices based on P_per_class! each class has P_per_class prototypes, so select indices from P
        # selected_indices = [i for i in range(P) if proto_class_identity[i] in selected_classes]
        # ############################################
        for j, prototype_j_dict in enumerate(prototypes_to_plot):
            # ############################################
            # # skip if the prototype is not selected
